...
// Radar.on('onGeofence', (result) => {
//     const { geofence, user, event } = result;
//     console.log(`Geofence event: ${event} in ${geofence.tag}`);
...
export default function App() {
  ...
  Radar.on("events", ({events}) => {
    ...
    setEvent(stringify(events[0].type))
  });
  ...
  useEffect(() => {
    ...
    // Radar.trackOnce()
    // .then(({events}) => {
    //    events && console.log("result event : ",events.type)
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{ width: "100%", height: "50%" }}>
          <ScrollView>
            <Text style={{color:"red",fontSize:20,borderWidth:0}}>Event : {event}</Text>
